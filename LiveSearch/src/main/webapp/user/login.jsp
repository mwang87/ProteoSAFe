...
%><%
	...
	boolean checkingLogin = request.getParameter("login") != null;
	boolean authenticated = checkingLogin && manager.authenticate(user, pswd);

	if(!checkingLogin) user="";
	if(from == null || from.isEmpty()) from = "/";
//	from.replaceFirst("^http://", "https://");
%>
...
<%
	if(authenticated){
		...
		if(credentials.get("hash") == null){
			manager.updateHash(user, pswd);
			FileManager.syncFTPSpace(user);
		}
%>
<div style="text-align: center;"><h1>Successful login</h1></div>
<%
	}
	else{
		if(checkingLogin)
...
<input type="hidden" name="url" value="<%= from %>"/>
</form>
<%	} %>
