...
extends BaseServlet
{
	...
	private static final Logger logger =
		LoggerFactory.getLogger(CleanupLabel.class);
	
	/*========================================================================
	 * Public interface methods
	 *========================================================================*/
	/**
	 * <p>Accepts and processes requests to clean up the label directories
	...
	throws ServletException, IOException {
		...
		if (isAuthenticated() == false) {
			response.sendError(HttpServletResponse.SC_UNAUTHORIZED,
				"You must be logged in to clean up task label directories.");
			return;
		}
		
		// delete all files in all task label directories
		String result = Cleanup.DeleteLabelFiles();
		if (result == null) {
			response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
				"There was an error cleaning up task label directories. " +
